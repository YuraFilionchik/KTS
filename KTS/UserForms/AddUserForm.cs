...
namespace KTS
{
    public partial class AddUserForm : Form
    {
        ...
        private void btAddDevToUser_Click(object sender, EventArgs e)
        {
            if(cbAllDevs.SelectedIndex==-1) return;
           
            var dev = DB.Devices.First(x => x.Name == cbAllDevs.Text.Trim());
            if(dev==null || SelectedUser==null)return;
            if(SelectedUser.UserDevices==null||SelectedUser.UserDevices.Contains(dev))return;
            DB.Users.Find(SelectedUser.UserId)?.UserDevices.Add(dev);
            DB.SaveChanges();
